...
const Features = () => {
 ...
 const itemVariants = {
  ...
  <section className="py-20 relative bg-black/40 backdrop-blur-md">
   ...
   >
   <h2 className="text-4xl font-bold mb-4 text-orange-300 glow">
   ...
   <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-0.5 bg-gradient-to-b from-orange-500/30 via-yellow-500/30 to-orange-700/30">
   </div>
   ...
   className="text-xl font-semibold mb-2 text-orange-300 glow"
   >
   {feature.title}
   </motion.h3>
   <p className="text-orange-200 glow">{feature.description}</p>
   </div>
   ...
   >
   <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${feature.color} flex items-center justify-center shadow-lg shadow-orange-500/30 text-white`}>
   {feature.icon}
   </div>
   </motion.div>
  ...
  <motion.div 
   ...
   className="bg-black/30 backdrop-blur-sm rounded-xl p-6 shadow-lg shadow-orange-500/10 border border-orange-500/20 group"
  >
   <div className="h-32 rounded-lg bg-gradient-to-br from-black to-orange-950/20 flex items-center justify-center overflow-hidden relative">
   <motion.div
    animate={{ opacity: [0.9, 1, 0.9], scale: [0.3] }}
    transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
    className="text-lg md:text-xl lg:text-2xl text-orange-400/70 glow p-4 rounded-lg bg-black/40 text-center font-mono tracking-wide"
   >
    {[
    ...
    <span key={i} className="text-orange-300">{line}</span>
    ))}
   </motion.div>
   ...
   <motion.div
    ...
    className="absolute -top-6 -right-6 w-12 h-12 rounded-full bg-gradient-to-br from-orange-500/20 to-transparent"
   />
   <motion.div
    animate={{ scale: [1, 1.3, 1], opacity: [0.1, 0.3, 0.1] }}
    transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: index * 0.3 }}
    className="absolute -bottom-4 -left-4 w-8 h-8 rounded-full bg-gradient-to-tr from-yellow-600/20 to-transparent"
   />
   </div>
  </motion.div>
  </div>
